...
const cleanUnusedImports = (text, options) => {
  ...
  try {
    ...
    mergeImpDeclarationsWithSameModuleSpecifier(sourceFile);
...
/**
 * Merges multiple import declarations with the same `moduleSpecifier` into a single unified import declaration.
 * @param {SourceFile} sourceFile - The source file to process and modify.
 */
const mergeImpDeclarationsWithSameModuleSpecifier = (sourceFile) => {
  const imports = sourceFile.getImportDeclarations();
  ...
  groupedImports.forEach((group, moduleSpecifier) => {
    ...
    group.forEach((importDeclaration) => {
      const currentNamespaceImport = importDeclaration.getNamespaceImport()?.getText();
      if (currentNamespaceImport) {
        if (namespaceImport || defaultImport || namedImports.size > 0) {
          console.warn(`Skipping namespace import conflict for ${moduleSpecifier}`);
        } else {
          namespaceImport = currentNamespaceImport;
        }
        return;
      }

      const currentDefaultImport = importDeclaration.getDefaultImport()?.getText();
      if (currentDefaultImport) {
        defaultImport = currentDefaultImport;
      }

      importDeclaration.getNamedImports().forEach((namedImport) => {
        namedImports.add(namedImport.getText());
      });
    });

    const defaultPart = defaultImport ? `${defaultImport}` : '';
    const namedPart = namedImports.size > 0 ? `{ ${[...namedImports].join(', ')} }` : '';
    const namespacePart = namespaceImport ? `* as ${namespaceImport}` : '';
    const separator = defaultPart && (namedPart || namespacePart) ? ', ' : '';
    const unifiedImport = `import ${defaultPart}${separator}${namedPart || namespacePart} from ${moduleSpecifier};`;
    group[0].replaceWithText(unifiedImport);
